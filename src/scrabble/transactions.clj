...
;; Recorded refs are references that represent a persistent entity.
;; They consist of the persistent ID of the entity and a ref to the
;; object.

(def id-counter (atom 0))

(defrecord RecordedRef [id ref]
  IDeref
  (deref [_] (deref ref)))

(defmethod print-method RecordedRef [value ^java.io.Writer writer]
  (.write writer "#RecordedRef")
  (.write writer (pr-str {:id (:id value)})))

;; Found in https://gist.github.com/rwilson/34c88a97c6260a7dc703
(defmethod pprint/simple-dispatch RecordedRef [o]
  ((get-method clojure.pprint/simple-dispatch clojure.lang.IPersistentMap) o))

(defn pref [value & args]
  (RecordedRef. (swap! id-counter inc) (apply ref value args)))
...
(defn wrap-transaction [signature body]
  `((let [return-value# (dosync ~@body)]
      (log-transaction ~@signature)
      return-value#)))

(defmacro deftx [& args]
  (let [{:keys [name] :as conf} (s/conform :scrabble.defn-specs/defn-args args)]
    (->> (update-conf conf (partial wrap-transaction (extract-signature conf)))
